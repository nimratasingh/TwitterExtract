...
public class PhoneExtractReducer extends Reducer<Text, Text,Text, Text>  {
	...
	public void reduce(Text key, Iterable<Text> values, Context context) throws IOException, InterruptedException{
		...
		if(product.isEmpty()){
			...
			Path path = new Path(context.getConfiguration().get("PHONE_RATING"));
			...
			while((line = reader.readLine())!= null){
				...
				if (lower.contains(camera)){
					lower = lower.replaceAll("phone", "NA");
