...
public class TabletExtractReducer extends Reducer<Text, Text,Text, Text> {
	...
	public void reduce(Text key, Iterable<Text> values, Context context) throws IOException, InterruptedException{
		...
		if(product.isEmpty()){
			...
			Path path = new Path(context.getConfiguration().get("TABLET_RATING"));
			...
			while((line = reader.readLine())!= null){
				...
				if (lower.contains(tablet)){
					lower = lower.replaceAll("tablet", "NA");
				}
